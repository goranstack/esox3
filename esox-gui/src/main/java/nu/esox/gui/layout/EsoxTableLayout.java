...
public class EsoxTableLayout implements LayoutManager2
{
    ...
    protected transient EsoxSizeRequirements[] m_widths;
    protected transient EsoxSizeRequirements[] m_heights;
    protected transient EsoxSizeRequirements m_width;
    protected transient EsoxSizeRequirements m_height;

    private static final boolean TRACE = false;


    public EsoxTableLayout() // two column table
    {
        this( 2 );
    }
    

    public EsoxTableLayout( int count ) // n column table with stretching last column
    {
        this( count, true, true, true, LAST, NONE, 0, 0 );
    }
    

    public EsoxTableLayout( int count, boolean isCountHorizontal )
    {
        this( count, isCountHorizontal, true, true, NONE, NONE, 0, 0 );
    }
    
    public EsoxTableLayout( int count, boolean isCountHorizontal, boolean fillCellsX, boolean fillCellsY, int fillColumn, int fillRow, int horizontalGap, int verticalGap )
    {
        m_count = count;
    ...
    void checkRequests( Container target )
    {
            ...
            ( m_width == null )
        {
            ...
            m_width = new EsoxSizeRequirements();
            m_height = new EsoxSizeRequirements();
            if
                ( m_isCountHorizontal )
            {
                m_widths = new EsoxSizeRequirements[ m_count ];
                m_heights = new EsoxSizeRequirements[ (int) Math.ceil( (float) I / m_count ) ];
            } else {
                m_widths = new EsoxSizeRequirements[ (int) Math.ceil( (float) I / m_count ) ];
                m_heights = new EsoxSizeRequirements[ m_count ];
            }
            for
                ( int i = 0; i < m_widths.length; i++ )
            {
               m_widths[ i ] = new EsoxSizeRequirements();
            }
            for
                ( int i = 0; i < m_heights.length; i++ )
            {
               m_heights[ i ] = new EsoxSizeRequirements();
            }
